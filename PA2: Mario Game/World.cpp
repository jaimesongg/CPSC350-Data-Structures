...
World::World() {
    ...
    moves = 0; // tracks number of moves Mario makes
    result = true; // end result (true if Mario wins, false if Mario dies)
}

World::~World() {
    delete mario; // frees memorry for Mario
    if (enemy != nullptr) {
        delete enemy; // frees memeory for enemy if it exists
    }
    delete fp; // frees file processor memory
}
...
World::World(string i, string o, int L, int N, int V, int coinP, int emptyP, int goombaP, int koopaP, int mushroomP) {
    ...
    // percentages for items on the level
    coinPer = coinP;
    emptyPer = emptyP;
    goombaPer = goombaP;
    koopaPer = koopaP;
...
// handles Mario's movement and interactions in a level
bool World::gameplay(int levelNum) {
    ...
    // creates a new level with given dimensions and item percentages
    Level *currLevel = new Level(dim, last, coinPer, emptyPer, goombaPer, koopaPer, mushroomPer);
    currLevel->buildLevel(); // generates level layout
     // displays initial level w/o Mario
    fp->border();
    ...
    fp->displayLevel(currLevel->convertToString()); // prints the level with Mario's position 
    currLevel->levelArray[marioRow][marioCol] = temp; // set the item back where Mario is to prep for while loop
    ...
    while (continueLevel) {
        ...
        randNum = 1 + rand() % 4; // generates a random direction (1 = up, 2 = right, 3 = down, 4 = left)
        
        // handles movement based on random direction
        if (randNum == 1) {
        ...
        // after Mario moves, updates posiiton in grid
    ...
    delete currLevel; // free memory for current level
    if (levelNum != dim - 1) {
        currLevel == nullptr;
    }
    return result;
